...
class EvidenceService:
    ...
    def from_sql_rows(
        ...
        node_ids: set[str] = set(additional_node_ids or [])
        edge_ids: set[str] = set()
        for row in rows:
            node_ids.update(self._node_ids_from_row(row))
            edge_ids.update(self._edge_ids_from_row(row))
        ordered_node_ids = self._graph_service.filter_existing_node_ids(sorted(node_ids))
        ordered_edge_ids = self._graph_service.filter_existing_edge_ids(sorted(edge_ids))
        cited_nodes = [
            CitationNode(
                id=node.id,
                type=node.type,
    ...
    def _node_ids_from_row(self, row: dict[str, Any]) -> set[str]:
        ...
        if row.get("customer_number") is not None:
            found.add(f"customer:{row['customer_number']}")
        return found

    def _edge_ids_from_row(self, row: dict[str, Any]) -> set[str]:
        edge_ids: set[str] = set()
        ...
        if sales_order and customer_id:
            edge_ids.add(f"ordered_by:{sales_order}:{customer_id}")
        if sales_order and sales_order_item:
            edge_ids.add(f"has_item:{sales_order}:{sales_order_item}")
        if sales_order and sales_order_item and product_id:
            edge_ids.add(f"refers_to_product:{sales_order}:{sales_order_item}:{product_id}")
        if sales_order and sales_order_item and production_plant:
            edge_ids.add(f"shipped_from_order_item:{sales_order}:{sales_order_item}:{production_plant}")
        if delivery_document and delivery_document_item:
            edge_ids.add(f"part_of_delivery:{delivery_document}:{delivery_document_item}")
        if delivery_document and customer_id:
            edge_ids.add(f"delivered_to:{delivery_document}:{customer_id}")
        if delivery_document and plant:
            edge_ids.add(f"shipped_from_delivery:{delivery_document}:{plant}")
        ...
        if billing_document and billing_document_item:
            edge_ids.add(f"part_of_billing:{billing_document}:{billing_document_item}")
